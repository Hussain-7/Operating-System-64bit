...
//the data we sent from the keyboard to the handler is not the key map we see here instead
//it is the scan code,which means we need to convert scan code to ascii code to print it on screen
...
static char keyboard_read(void)
{
    ...
    char ch;
                //To read a scan code we read from port 0x60
